...
public class CPAUtility {
	 ...
	 */
	public HashMap<String, Set<CriticalPairNode>> persistCpaResult(CPAResult cpaResult, String path) {
		...
		for (CriticalPair cp : cpaResult) {
			...
			String numberedNameOfCPKind = "(" + numberForRulePair + ") " + criticalPairKind;
	 ...
	 */
	private CriticalPairNode persistSingleCriticalPair(CriticalPair cp, String numberedNameOfCriticalPair, String path) {
		...
		EGraph minimalModel = cp.getMinimalModel();
				...
				for (Node nacNode : nacNodes) {
					if (match.getNodeTarget(nacNode) != null) {
						...
						if (hashToName.containsKey(overlapNodeHash)) {
							...
							List<EObject> root = minimalModel.getRoots();

							EPackage epack = (EPackage) root.get(0);

							EList<EClassifier> eclasses = epack.getEClassifiers();
	 ...
	 */
	private void renameNodesOfMinimalModel(EGraph minimalModel, Map<Integer, String> hashValueToNameMapping) {
		List<EObject> root = minimalModel.getRoots();
		EPackage epack = (EPackage) root.get(0);
		EList<EClassifier> eclasses = epack.getEClassifiers();
		for (EClassifier eclass : eclasses) {
	 ...
	 */
	private URI saveMinimalModelInFileSystem(ResourceSet resourceSet, EGraph minimalModel, String fullPathMinimalModel) {
		URI overlapURI = URI.createFileURI(fullPathMinimalModel);
		Resource overlapres = resourceSet.createResource(overlapURI, "ecore");

		for (EObject elem : minimalModel) {
			overlapres.getContents().add(elem);
		}

		try {
			overlapres.save(null);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return overlapURI;
	 ...
	 * @return the <code>URI</code> of the saved file.
	 */
	private URI saveRuleInFileSystem(ResourceSet resourceSet, Rule rule, String fullFilePath) {
	 ...
	 */
	private int processLhsOrRhsOfRuleForPersisting(EGraph minimalModel, Graph LHS_or_RHS_ofRule,
		...
		for (Node elem : LHS_or_RHS_ofRule.getNodes()) {
			if (match.getNodeTarget(elem) != null) {
				...
				if (hashToName.containsKey(overlapNodeHash)) {
					...
					// resolve the list of nodes of the created minimal graph
					List<EObject> root = minimalModel.getRoots();
					EPackage epack = (EPackage) root.get(0);
					EList<EClassifier> eclasses = epack.getEClassifiers();
