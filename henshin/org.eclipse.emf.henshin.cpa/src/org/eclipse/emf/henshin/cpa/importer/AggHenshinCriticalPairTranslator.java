...
//	private Module module;
	...
	private void processAGGresultOfRulePair(CPAResult result, Rule rule1, Rule rule2, CriticalPairData cpd) {
		...
		if (validCriticalPair) {
			...
			org.eclipse.emf.henshin.interpreter.EGraph henshinOverlap = new org.eclipse.emf.henshin.interpreter.impl.EGraphImpl();
			henshinOverlap.add(cpaEPackage);

			if (criticalPairType == CPType.Dependency) {
				Dependency dep = new Dependency(firstHenshinRuleOriginal, secondHenshinRuleOriginal, henshinOverlap,
				...
				Conflict conf = new Conflict(firstHenshinRuleOriginal, secondHenshinRuleOriginal, henshinOverlap,
				...
				if (memberAt.getHoldingTuple() instanceof ValueTuple) {
					...
					for (EStructuralFeature eStructFeat : eStructuralFeatures) {
						if (eStructFeat instanceof EAttribute && !dontProcessThisAttribute) {
							EAttribute allreadyEsitingEAttribute = (EAttribute) eStructFeat;
							// check if eAttribute is already processed
							if (allreadyEsitingEAttribute.getName().equals(newAttrForMinimalGraph.getName())) {
								dontProcessThisAttribute = true;
								break;
							}
						}
