...
public abstract class CriticalPair {
	...
	CriticalPair(Rule r1, Rule r2, EGraph minimalModel) {
		this.r1 = r1;
		this.r2 = r2;
	 ...
	 */
	public EGraph getMinimalModel() {
		return minimalModel;
	}

	/**
	 * Adds critical elements to the list of critical elements of this critical pair, given that each critical pair is complete.
	...
	public boolean addCriticalElements(List<CriticalElement> newCriticalElements) {
		...
		for(CriticalElement criticalElement : newCriticalElements){
			if(criticalElement.commonElementOfCriticalGraph == null || criticalElement.elementInFirstRule == null || criticalElement.elementInSecondRule == null){
				...
				System.err.println("A critical element of the rules Ñ"+getFirstRule().getName()+"ì -> ì"+getSecondRule().getName()+"î is incomplete!");
				System.err.println("The following parts of the critical element are missing:");
				if(criticalElement.commonElementOfCriticalGraph == null){
					System.err.println("The critical element from within the AGG result.");
				}
				if(criticalElement.elementInFirstRule == null){
					System.err.println("The occurrence of the critical element in the first rule.");
				}
				if(criticalElement.elementInSecondRule == null){
					System.err.println("The occurrence of the critical element in the second rule.");
				}
			}
			else {
