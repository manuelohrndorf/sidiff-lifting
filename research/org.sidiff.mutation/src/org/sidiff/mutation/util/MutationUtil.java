package org.sidiff.mutation.util;
...
import org.sidiff.mutation.Mutation;
	...
	public static void adaptContext(Copier copier, Match context, Rule rule){
		...
		for(Node node : rule.getLhs().getNodes()){
			if(copier.containsKey(context.getNodeTarget(node))){
				...
				// Get old value and copy annotation to new target
				AnnotateableElement annotElem = EMFAdapter.INSTANCE.adapt(oldTarget , AnnotateableElement.class);				
				if(annotElem.hasAnnotation(SimilarityContextSelection.KEY_SELECTED)){
					int simValue = annotElem.getAnnotation(SimilaritySelection.KEY_SELECTED,int.class);
					AnnotateableElement annotElemCopy = EMFAdapter.INSTANCE.adapt(newTarget , AnnotateableElement.class);
					annotElemCopy.setAnnotation(SimilaritySelection.KEY_SELECTED, simValue);
					LogUtil.log(LogEvent.DEBUG, "Copied similarity annotation value(" + simValue + ") from " + oldTarget + " to " + newTarget);	
