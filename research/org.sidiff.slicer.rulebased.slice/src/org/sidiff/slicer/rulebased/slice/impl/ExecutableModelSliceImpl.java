...
public class ExecutableModelSliceImpl extends ModelSliceImpl implements ExecutableModelSlice {
	 ...
	 * @generated
	 */
	public void serialize(String path, final boolean zip) {
		String modelslice_name = "ExecutableSlice.slice";  String symmetricdiff_name = "ExecutableSlice." + AsymmetricDiffFacade.SYMMETRIC_DIFF_EXT;  String asymmetricdiff_name = "ExecutableSlice." + AsymmetricDiffFacade.ASYMMETRIC_DIFF_EXT;  String foldername = "RuleBasedSlice";  String separator = File.separator;  for(SlicedElement slicedElement : this.getSlicedElements()) { slicedElement.setObject(null); }  if (!path.endsWith(separator)) { path += separator; }  path += foldername;  SymmetricDifference symmetricDifference = this.getAsymmetricDifference().getSymmetricDifference(); symmetricDifference.getMatching().setUriA(modelslice_name); symmetricDifference.getMatching().setUriB(modelslice_name); symmetricDifference.setUriModelA(modelslice_name); symmetricDifference.setUriModelB(modelslice_name);  AsymmetricDifference asymmetricDifference = this.getAsymmetricDifference(); asymmetricDifference.setUriOriginModel(modelslice_name); asymmetricDifference.setUriChangedModel(modelslice_name);  ResourceSet resourceSet = new ResourceSetImpl();  String resSymDiffSavePath = path + separator + symmetricdiff_name;  resourceSet.createResource(EMFStorage.pathToUri(resSymDiffSavePath)).getContents().add(symmetricDifference);    String resAsymDiffSavePath = path + separator + asymmetricdiff_name;  resourceSet.createResource(EMFStorage.pathToUri(resAsymDiffSavePath)).getContents().add(asymmetricDifference);   String resModelSliceSavePath = path + separator + modelslice_name;  resourceSet.createResource(EMFStorage.pathToUri(resModelSliceSavePath)).getContents().add(this);  for(Resource resource : resourceSet.getResources()) { try { resource.save(null); } catch (IOException e) { e.printStackTrace(); } }  if(zip) { ZipUtil.zip(path, path, AsymmetricDiffFacade.PATCH_EXTENSION); FileOperations.removeFolder(path); }
	}
	...
	public Object eInvoke(int operationID, EList<?> arguments) throws InvocationTargetException {
		switch (operationID) {
			case RuleBasedSlicePackage.EXECUTABLE_MODEL_SLICE___SERIALIZE__STRING_BOOLEAN:
				serialize((String)arguments.get(0), (Boolean)arguments.get(1));
				return null;
		}
