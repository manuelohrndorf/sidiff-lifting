...
public class OCLConstraintInterpreter implements IConstraintInterpreter {
	...
	@Override
	public ConstraintResult evaluate(Constraint constraint, EObject context) {
	...
	public IContentProposal[] getSyntaxHelp(EObject context, String expression, int cursorPosition)
	{
		...
		expression = expression.substring(0, cursorPosition);
		...
		List<Choice> choices = this.oclHelper.getSyntaxHelp(ConstraintKind.INVARIANT, expression);
		IContentProposal syntaxHelp[] = new IContentProposal[choices.size()];
		for(int i = 0; i < choices.size(); i++)
		{
			...
			syntaxHelp[i] = new ContentProposal(content, label, null, newCursorPosition);
		}
