...
import static org.eclipse.emf.henshin.editing.utils.util.HenshinModelHelper.isNodeWithChangingAttributes;
import static org.eclipse.emf.henshin.editing.utils.util.HenshinModelHelper.isNodeWithCreationEdges;
import static org.eclipse.emf.henshin.editing.utils.util.HenshinModelHelper.isNodeWithDeletionEdges;
		...
		if (editRule != null) {
			...
			editRule.eAllContents().forEachRemaining(element -> {
				if (element instanceof Node) {
					...
					if (lhsNode.getGraph().isLhs()) {
						Node rhsNode = getRemoteNode(lhsNode.getGraph().getRule().getMappings(), lhsNode);

						// Is preserve node?
					if (rhsNode != null) {
						if (!isNodeWithCreationEdges(rhsNode) && !isNodeWithDeletionEdges(lhsNode)
								&& !isNodeWithChangingAttributes(lhsNode) && !isNodeWithChangingAttributes(rhsNode)) {
