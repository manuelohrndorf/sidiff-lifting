	...
	public static final class AttributeChangeImpl extends ChangeImpl implements AttributeChange {
		...
		public void applyAndReverse() {
			...
			if (!initialized) {
				...
				//   Undo fails with NullPointerException if a Henshin rule creates
				//   an object and explicitly sets any attribute value to its default
				//   value. That means [oldValue.equals(newValue) == true] and thus 
				//   attribute is set to null (which is, I guess, not right).
				if (attribute==null) {
					return;
				}
