...
import org.sidiff.integration.compare.SiLiftCompareDifferencer;
import org.sidiff.integration.compare.SiLiftCompareDifferencer.IModelViewerAdapter;
...
public class ShowDiagramAction extends Action {
	...
	public ShowDiagramAction(SiLiftCompareConfiguration config) {
		...
		this.setImageDescriptor(CompareIntegrationPlugin.getImageDescriptor(CompareIntegrationPlugin.IMAGE_SHOW_DIAGRAM));
	...
	public void run() {
		MessageDialogUtil.showProgressDialog(new IRunnableWithProgress() {
			...
			public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				MultiStatus multiStatus = new MultiStatus(CompareIntegrationPlugin.ID, 0, "Error showing diagrams", null);
	...
	protected IEditorPart openDiagram(String label, CompareResource res) throws CoreException {
		if(res.getResource() == null) {
			throw new CoreException(new Status(IStatus.ERROR, CompareIntegrationPlugin.ID,
		...
		try {
			...
			StatusManager.getManager().handle(e, CompareIntegrationPlugin.ID);
		}
	...
	protected void changePartName(IEditorPart part, String name) throws CoreException {
		try {
			...
			throw new CoreException(new Status(IStatus.WARNING, CompareIntegrationPlugin.ID, "Changing part names failed", e));
		};
	}
