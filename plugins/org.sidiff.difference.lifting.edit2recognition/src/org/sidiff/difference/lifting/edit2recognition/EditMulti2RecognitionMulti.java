...
public class EditMulti2RecognitionMulti extends EditUnit2RecognitionUnit {
	...
	private void createCorrespondenceMappings(
		...
		for (Mapping mapping : getPreservationNodeMappings(editKernel, editMulti)) {
			...
			if (kernelPattern.getNode_a() != null) {
				// Map Model A node
				createMultiMapping(recognitionMulti, kernelPattern.getNode_a(), multiPattern.getNode_a());
			}

			if (kernelPattern.getNode_b() != null) {
				// Map Model B node
				createMultiMapping(recognitionMulti, kernelPattern.getNode_b(), multiPattern.getNode_b());
			}
	...
	private void createAddObjectMappings(
		...
		for (Mapping mapping : getCreationNodeMappings(editKernel, editMulti)) {
			...
			// Map Model A node
			createMultiMapping(recognitionMulti,
					kernelPattern.getNode_b(), multiPattern.getNode_b());
		}
