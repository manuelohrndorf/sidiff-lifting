 ...
 */
// FIXME: Consider containment/container references!
// FIXME: Consider opposites which are missing in the graph pattern!
...
//		printNodes(nodes);
	 ...
	 * algorithm ({@link #repairSorting(int, Integer, List)}) searches for the
	 ...
	 * ({@link #repairSorting(int, Integer, List)}) checks the sorting
	 ...
	 */
	private static int repairSorting(int i, int offset, EList<Node> nodes) {
		...
		if (!checkSortingConstraint(i, nodes)) {
			...
			if (j > 0) {
				nodes.move(i, j);
				offset = repairSorting(i, offset + 1, nodes);
			} else {
				System.err.println("Optimal sorting impossible: " + nodes.get(i));
			}
		}
