...
public class LiftingGraphEngine extends SelfCleaningEngineImpl {
	 ...
	 */
	private boolean reverseCheckMatching = true;

	/**
	 * Index and reverse check meta-types from the model to the model difference
	 * during matching.
	 */
	private boolean reverseCheckMetaTypes = true;
	...
	public RuleInfo getRuleInfo(Rule rule) {
		...
		if (!ruleInfos.containsKey(rule)) {
			...
			if (reverseCheckMatching || reverseCheckMetaTypes || reverseCheckDifference) {
			...
			if (incoming.getType() == SYMMETRIC_PACKAGE.getAddObject_Obj()
				...
				return new InverseCrossReferenceConstraint(target, incoming) {
					...
					public List<EObject> getInverseCrossReferenced(EObject source, Edge incoming) {
						...
						Iterator<? extends Change> changeIterator = changeIndex.getLocalChanges(source, incoming.getType(),
	...
	public boolean isReverseCheckMatching() {
		return reverseCheckMatching;
	}

	public void setReverseCheckMatching(boolean reverseCheckDifference) {
		this.reverseCheckMatching = reverseCheckDifference;
	}
	
	public boolean isReverseCheckMetaTypes() {
		return reverseCheckMetaTypes;
	}

	public void setReverseCheckMetaTypes(boolean reverseCheckMetaTypes) {
		this.reverseCheckMetaTypes = reverseCheckMetaTypes;
	}
	...
	public void setReverseCheckDifference(boolean reverseCheckDifferenceChanges) {
		this.reverseCheckDifference = reverseCheckDifferenceChanges;
	}
